...
function SroTransferLand() {
  ...
  const newOwnerChangeHandler = (e) => {
    settransferLandData((transferLandData) => ({
      ...transferLandData,
      newOwner: e.target.value,
    }));
  };

  const newOwnerAadhaarChangeHandler = (e) => {
    settransferLandData((transferLandData) => ({
      ...transferLandData,
      newOwnerAadhaar: e.target.value,
    }));
  };

  console.log(transferLandData);
  ...
  const verifyButtonHandler = async () => {
    console.log(transferLandData.newOwnerAadhaar);
    let regex = /^\d{12}$/;
    let isRegexPassed = regex.test(transferLandData.newOwnerAadhaar);

    // if (transferLandData.newOwner && transferLandData.newOwnerAadhaar && isRegexPassed) {
    if (transferLandData.newOwner) {
      transferLandData.landId = landReference;
      setisLoading(true);
      try {
        const response = await fetch(
          `http://localhost:4000/api/blockChainTransferLand`,
          {
            method: "post",
            body: JSON.stringify({
              transferLandData,
          ...
          if (response.status === 200) {
            swal(
              "Success",
              `Updated Land  ${landReference} Owner to blockchain successfully`,
              "success"
            );
            resetButtonHandler();
          } else {
    ...
    } else if (!transferLandData.newOwner) {
      swal("Error", `Please enter the new owner name`, "error");
    }
    // else if (!transferLandData.newOwnerAadhaar) {
    //   swal(
    //     "Error",
    //     `Please enter the new owner aadhaar no.(mandatory)`,
    //     "error"
    //   );
    // } else {
    //   swal("Error", `Please enter the valid aadhaar no.`, "error");
    // }
  };
    ...
    <div className="sro-background">
      ...
      <div className="sro-searchbox">
        <Stack gap={3} className="col-md-5 mx-auto">
          <h1>Transfer Land Ownership in Blockchain</h1>
            ...
            <Table aria-label="simple table">
              ...
              <TableBody>
                ...
                {/* <TableRow>
                  ...
                  <TableCell align="right">{searchedLandData.landId}</TableCell>
                </TableRow> */}
                <TableRow>
                  <TableCell component="th" scope="row">
                    Existing Owner
                ...
                <TableRow>
                  ...
                  <TableCell align="right">
                    <TextField size="small" onChange={newOwnerChangeHandler} />
                  </TableCell>
                </TableRow>
                {/* //TODO: */}
                <TableRow>
...
export default SroTransferLand;
