...
import RevenueNavbar from "./RevenueNavbar";
...
function RevenueMutation() {
  ...
  const mutationStatusChangeHandler = (e) => {
    setmutationStatus(e.target.value);
  };
  ...
  const verifyButtonHandler = async () => {
    if (mutationStatus) {
      let mutationObject = {};
      if (mutationStatus === "mutationDone") {
        mutationObject["mutationStatusUpdate"] = true;
      } else if (mutationStatus === "mutationRejected") {
        mutationObject["mutationStatusUpdate"] = "rejected";
      }
      mutationObject.landId = landReference;

      setisLoading(true);
      try {
        const response = await fetch(
          `http://localhost:4000/api/updateMutationStatus`,
          {
            method: "post",
          ...
          if (response.status === 200) {
            swal(
              "Success",
              `Updated Land  ${landReference} Mutation Status to blockchain successfully`,
              "success"
            );
            resetButtonHandler();
          } else {
      ...
      swal("Error", `Please update the mutation status`, "error");
    }
  };

  return (
    <div className="sro-background">
      <RevenueNavbar page="Mutate Land Record" />
      <div className="sro-searchbox">
        <Stack gap={3} className="col-md-5 mx-auto">
          <h1>Add Land Mutation in Blockchain</h1>
              ...
              <TableBody>
                ...
                searchedLandData.isLandMutated == "true" ? (
                  <TableRow>
                    ...
                    <TableCell align="right">
                      <b>Land Mutation Already Done</b>
                    </TableCell>
                  </TableRow>
                ) : (
                  <TableRow>
                    ...
                    <TableCell align="right">
                      <TextField
                        select
                        size="small"
                        onChange={mutationStatusChangeHandler}
                      >
                        <MenuItem value={"mutationDone"}>
                          Mutation Done
                        </MenuItem>
                        <MenuItem value={"mutationRejected"}>
                          Mutation Rejected
                        </MenuItem>
                      </TextField>
                    </TableCell>
